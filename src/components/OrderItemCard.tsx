...
const OrderItemCard = ({ order }: Props) => {
    ...
    const getTime = () => {
        ...
        const min = orderDateTime.getMinutes()
    ...
    return (<Card>
        ...
        <CardContent className="flex flex-col gap-6">
            <div className="flex flex-col gap-2">{order.cartItems.map((cartItem, index) => (<span key={index}><Badge variant="outline" className="mr-2">{cartItem.quantity}</Badge>
                {cartItem.name}</span>))}</div>
            <div className="flex flex-col space-y-1.5">
                ...
                <Select value={status} disabled={isLoading} onValueChange={(value) => handleStatusChange(value as OrderStatus)}>
                    ...
                    <SelectContent position="popper">
                        {ORDER_STATUS.map((status, index) => (<SelectItem className="cursor-pointer" key={index} value={status.value}>{status.label}</SelectItem>))}
                    </SelectContent>
                </Select>
            </div>
        </CardContent>
